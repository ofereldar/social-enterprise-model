...
\maketitle
\begin{abstract}
This paper studies how informational asymmetries about beneficiaries'
abilities and needs shape organizational choice. The entrepreneur chooses
legal form and whether to help beneficiaries through giving or through
transactions with them. Transactions with beneficiaries generate information
that allows disbursals to be tailored to need. The central implication
is paradoxical: the for-profit form arises where philanthropic quality
matters most. When beneficiary heterogeneity is high, transacting
aligns profit with mission, and entrepreneurs choose the for-profit
...
of giving. Certification of beneficiary transactions can help social
enterprises scale.
\end{abstract}
$\,$
...
Social enterprises pursue social goals through market activity. Some
are nonprofits, but many others operate as for-profits while nevertheless
training disadvantaged workers or sourcing from low-income producers.
This mix of mission and market
activity raises a basic organizational question: why do for-profit
social enterprises arise precisely in settings where effective philanthropy
appears to require the greatest concern for the quality of disbursals?

This paper studies a distinction often obscured in broad discussions
...
with them. The most straightforward application in the model is a work-integration
firm that can either fund outside training or hire and train disadvantaged
workers inside the firm. Fair-trade organizations and microfinance
institutions can illustrate the same broad mechanism: repeated transactions
with low-income farmers or borrowers can reveal information that helps
tailor assistance. What matters for the mechanism is repeated interaction
with beneficiaries, learning about their true abilities and needs,
and using that information to tailor disbursals.\footnote{For a discussion of the definition of social enterprise and of the
boundaries of the term in this paper, see \secref{oa:def-social-enterprise}
of the Online Appendix.}

The core mechanism is informational. Beneficiaries differ in ability
or need, but those differences are not readily observable ex ante.
An organization that only gives lacks a market platform on which to
learn who needs what. By contrast, an organization that transacts with
beneficiaries can observe performance, repayment, or output, infer
beneficiaries' attributes, and use that information to tailor assistance.
Throughout the paper, ``quality'' refers to the quality of philanthropy
in this narrow sense: the extent to which disbursals are matched to
beneficiaries' needs, not product quality.

Figure 1 organizes the four benchmark forms considered here: for-profit
or nonprofit, and giving or transacting. The contribution of the paper
...
The model has one entrepreneur who chooses a legal form, the number
of beneficiaries served, and whether each beneficiary is served through
giving or through a transaction with the firm. If the entrepreneur
transacts with a beneficiary, she can incur a measurement cost to learn
that person's true ability and tailor assistance. If she gives instead, assistance
must be based on the average beneficiary. The timing is simple: first
the entrepreneur chooses legal form and scale, including how many
beneficiaries to transact with; then the underlying abilities of transacted-with
beneficiaries are learned; finally the entrepreneur chooses disbursals
...
These results speak to classic nonprofit theory. \citet{hansmann1980} and
\citet{glaeserShleifer2001} emphasize the nondistribution constraint
as a device for protecting quality when customers or donors cannot
verify performance. I keep that insight, but change the object of
interest. Here the relevant quality problem is not generic shirking
on service quality; it is the misallocation of disbursals when beneficiaries'
needs differ and are hard to observe. In that environment, repeated
transactions with beneficiaries can align profit and mission by creating
information. The paper also
relates to recent influential models of socially oriented firms, including \citet{baron2007},
\citet{besleyGhatak2017}, \citet{deBettigniesRobinson2018},
and \citet{kaulLuo2018}, but emphasizes a different mechanism: beneficiary-facing
transactions as a source of information that improves the quality of
philanthropy.

It is important to emphasize that the legal-form results are not intended
to suggest that nonprofits are universally unimportant.
Rather, the model identifies a region in which the informational gains
from transacting with beneficiaries can substitute for the disciplining
role of the nondistribution constraint. The central implication is that
when philanthropic quality matters most, the
entrepreneur chooses the for-profit form while serving fewer beneficiaries
rather than expanding philanthropy through low-quality giving. The
nonprofit form matters mainly when beneficiary heterogeneity is lower
and assistance still takes the form of giving. This is not the paper's
main result, because in many development settings the main issue is
limited information coupled with beneficiary heterogeneity, so the
central problem is the quality of disbursals.
...
may be able to verify a narrower and more realistic object: whether
...
model and clarifies the meaning of quality. \secref{sec:analysis} analyzes
...
information. Or she can help beneficiaries through transactions with
them, which reveal beneficiary-specific information but require costly
measurement. The central tradeoff is therefore between scale and targeting.

There is one entrepreneur and a continuum of potential beneficiaries.
Each beneficiary $i$ has ability $a(i)<\bar{a}$, where $\bar{a}$
is the ability of a standard worker. Define beneficiary $i$'s need
as
...
Abilities are drawn from a distribution with mean $\mathbb{E}(a)$
and variance $Var(a)$. The entrepreneur does not observe individual
ability ex ante. I abstract from selection into the beneficiary pool:
beneficiaries are ex ante identical to the entrepreneur, and the entrepreneur
chooses how many beneficiaries to assist rather than which hidden
types to select.

The entrepreneur chooses total scale $n$. Of these $n$ beneficiaries,
$m$ are served through transactions with the firm and $g$ are served
through giving, so
...
The entrepreneur also chooses disbursals $d(i)$ and legal form. The
firm has fixed productive capacity $w$; $w$ is not a choice variable.
If the firm transacts with $m$ disadvantaged beneficiaries, the remaining
$w-m$ jobs are filled by standard workers. The analysis first solves
the entrepreneur's allocation problem for a fixed legal-form parameter
$v$, and then compares legal forms. A for-profit has $v=1$. A nonprofit
has $v<1$, reflecting the nondistribution constraint as a reduced-form
limit on the monetary payoff the entrepreneur can appropriate.

The two assistance technologies differ only in what the entrepreneur
learns. If the entrepreneur helps $g$ beneficiaries through giving,
she does not observe their individual abilities and chooses disbursals
...
serves $m$ beneficiaries through transactions, she can incur measurement
cost $C(m)$ to observe their abilities and choose disbursals $d^{m}(i)$.
The cost function $C(m)$ is increasing, convex, twice differentiable,
and satisfies $C(0)=0$.\footnote{As shown in \secref{oa:linear-cost} of the Online Appendix, the analysis is robust
to a linear cost function.} The work-integration interpretation is
that transactional beneficiaries are hired and trained inside the
firm. The broader interpretation is that a repeated market relationship,
such as employment, sourcing, or lending, creates information about
beneficiaries' abilities and needs.

Disbursals raise ability one-for-one up to $\bar{a}$. Thus, the
tailored disbursal for beneficiary $i$ is $x(i)=\bar{a}-a(i)$.
A lower disbursal leaves the beneficiary below $\bar{a}$; a higher
disbursal is wasteful. If the firm employs only standard workers,
revenue is $rw\bar{a}$. If it transacts with $m$ disadvantaged beneficiaries,
revenue is

\[
R(a)=r(w-m)\bar{a}+r\cdot\mathbb{E}\int_{0}^{m}\min\{a(i)+d^{m}(i),\bar{a}\}di.
\]

Giving beneficiaries are assisted outside the firm and do not affect
productive revenue.

Donations depend on promised scale $n$. I write the donation schedule
as $D(n)$, where $D(n)$ is increasing, concave, twice differentiable,
and satisfies $D(0)=0$. This reduced-form schedule summarizes a setting
in which donors can observe promised scale more easily than they can
observe the quality of disbursals ex ante \citep{charlesKim2016,coupetSchehl2022}.
It can also be viewed as a tractable simplification of a fuller donor
problem in which donor utility takes the form $U^{D}=Y-D(n)+F(n)$,
where $F(n)$ is increasing and concave in the number of beneficiaries
assisted. In the baseline model, the entrepreneur chooses $n$ and
takes $D(n)$ as given. \secref{subsec:donor-decision} later relaxes
this by allowing donors to care directly about expected quality.
Donations may come from external donors, consumer premia, investors
accepting below-market returns, workers accepting lower salaries,
or the entrepreneur herself.

Quality means targeting accuracy, not product quality. Formally,

\[
q=-\int_{0}^{n}\left(d(i)-x(i)\right)^{2}di
=-\int_{0}^{n}\left(d(i)-(\bar{a}-a(i))\right)^{2}di.
\]

Quality is maximized at $q=0$, which occurs when every beneficiary
receives exactly the disbursal needed to reach $\bar{a}$. The quadratic
loss treats over-provision and under-provision symmetrically and should
be read as a reduced-form measure of matching accuracy.\footnote{\secref{oa:no-penalty} of the Online Appendix discusses a quality measure with no penalty for waste. The conclusion discusses the case in which quality is defined by quantity.}
...
the entrepreneur's weight on effective assistance. Thus, $\gamma$
captures altruism, reputational pressure, or both. In contrast, \citet{glaeserShleifer2001}
assume that the altruistic component of the entrepreneur's utility
increases with the volume of disbursals, regardless of whether the
beneficiaries actually benefit.\footnote{The entrepreneur in \citet{glaeserShleifer2001} chooses $q$, the quantity
of disbursal, and her utility is given by $D-c(q)+\gamma\times q$,
where $D$ is a constant independent of $q$ because donors cannot
observe quantity when making their donations.}

The timing is:
\begin{itemize}
\item Before choices are made, nature draws beneficiary abilities from
the distribution of $a$. The entrepreneur observes the distribution
but not individual abilities.
\item At $t=0$, the entrepreneur chooses legal form, total scale $n$,
and the division of beneficiaries between transactions ($m$) and giving
($g=n-m$).
\item At $t=1$, the entrepreneur observes the abilities of transactional
beneficiaries after incurring $C(m)$, chooses disbursals $d^{m}(i)$
and $d^{g}(i)$, and payoffs are realized.
\end{itemize}

An important feature of social enterprises is the alignment of the
social entrepreneur's profit and social goals. To see this, rewrite
$q$ as: $q=-\int_{0}^{m}\left(d^{m}(i)-(\bar{a}-a(i))\right)^{2}di-\int_{0}^{g}\left(d^{g}(i)-(\bar{a}-a(i))\right)^{2}di$.
Maximum philanthropic quality is achieved when $d(i)=\bar{a}-a(i)$ $\forall i$,
such that $\ensuremath{q}=0$. If the entrepreneur employs $m$ individuals
with abilities below $\bar{a}$, the firm\textquoteright s earnings
decline because only $w-m$ workers have abilities $\bar{a}$. The
entrepreneur can improve the performance of these $m$ beneficiaries
by providing them with disbursal $d(i)=\bar{a}-a(i)$, increasing
their abilities to $\bar{a}$. Accordingly, by transacting with beneficiaries
and setting $d(i)=\bar{a}-a(i)$ for each $i$, the firm maximizes
revenues and improves the quality of disbursals.

In summary, the entrepreneur's maximization problem is:

\begin{equation}
\max_{m,g,d(i)}\;v\left[R(a)+D(n)-\mathbb{E}\int_{0}^{m}d^{m}(i)di-\mathbb{E}\int_{0}^{g}d^{g}(i)di-C(m)\right]+\gamma q\;\mbox{ s.t. }\;\;n=m+g,\label{eq:-1}
\end{equation}

where $R(a)$ depends on $m$ and $d^{m}(i)$ is chosen by the entrepreneur:

\[
R(a)=r(w-m)\bar{a}+r\cdot\mathbb{E}\int_{0}^{m}\min\{a(i)+d^{m}(i),\bar{a}\}di,
\]

and the altruistic component of the utility function is:

\[
\gamma q=-\gamma\cdot\mathbb{E}\left[\int_{0}^{m}\left(d^{m}(i)-(\bar{a}-a(i))\right)^{2}di\right]-\gamma\cdot\mathbb{E}\left[\int_{0}^{g}\left(d^{g}(i)-(\bar{a}-a(i))\right)^{2}di\right].
\]
...
\begin{equation}
d^{g*}(i)=\bar{a}-\mathbb{E}(a)-\frac{v}{2\gamma}\;\;\forall i\in[0,g].\label{eq:2}
\end{equation}
...
$\gamma\rightarrow\infty.$\footnote{To see why equation $\left(\ref{eq:2}\right)\;$holds, suppose the
entrepreneur sets $d^{*}(i)=\bar{a}-\mathbb{E}(a)-\frac{v}{2\gamma}-\varepsilon$
for $j$ beneficiaries, and the amount in equation $\left(\ref{eq:2}\right)$
for all other beneficiaries, where $\varepsilon>0$. The entrepreneur's
utility in this scenario would increase by $j\times\varepsilon$ due
to the savings in disbursals, but decrease by $j\left(\varepsilon+\varepsilon^{2}\right)$
due to the loss in quality. } Thus, the more the entrepreneur values effectiveness, the closer
...
I next solve for $d^{m*}(i)$, conditional on the entrepreneur transacting
with beneficiaries and incurring measurement costs such that she observes
$a$.
\begin{lem}\label{lem:tailoring}
If the entrepreneur incurs $C(m)$ to measure transactional beneficiaries,
she has both financial and altruistic incentives to tailor disbursals
to beneficiaries\textquoteright{} needs at t=1, such that, $d^{m*}(i)=\bar{a}-a(i)$$\;\;\forall i\in[0,m]$.
\end{lem}
The intuition is that if $d^{m}(i)<\bar{a}-a(i)$, the entrepreneur
...
If the entrepreneur chooses to transact with beneficiaries, she will
always incur measurement costs and tailor disbursals to their abilities.
\end{lem}
If the entrepreneur transacts with beneficiaries but does not measure
their abilities, she sets $d$ without knowing their true $a$. In
this case, $d$ is chosen as if the entrepreneur is engaging through
giving, though she has slightly stronger incentives to provide a higher
disbursal since her revenues depend on the beneficiary's productivity.
However, because the disbursal is not tailored to the beneficiary's
abilities, it may be too small or too large. If the disbursal is insufficient
to help the beneficiary reach the capabilities of standard workers
($\bar{a}$), the entrepreneur loses revenue due to poor performance.
Conversely, if the disbursal exceeds what is necessary to help the
beneficiary reach $\bar{a}$, it results in waste. Transacting without
measurement therefore combines the production cost of employing disadvantaged
beneficiaries with the same targeting problem that arises under giving.
This result shows why transactions matter in the model: once the entrepreneur
serves beneficiaries through a productive relationship, her profit
and mission incentives both point toward learning their needs and
tailoring disbursals.

The remaining choice is how many beneficiaries to serve through each
technology. Each increase in scale $n$ can be implemented either
by increasing $m$ or by increasing $g$. Increasing $m$ expands scale
through transactions: the entrepreneur receives the marginal donation,
pays the expected disbursal, and pays the marginal measurement cost.
Increasing $g$ expands scale through giving: the entrepreneur receives
the marginal donation and pays the expected disbursal, but also bears
the expected quality loss from untailored assistance. Letting $n=n'$,
$m=m'$, and $g=g'$, the entrepreneur's ex ante utility in equation
(\ref{eq:-1}) can be rewritten as:
...
\]
The entrepreneur will raise $m$ only if
\[
C'(m')<\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}
\quad\text{and}\quad
C'(m')<D'(n')-\left(\bar{a}-\mathbb{E}(a)\right).
\]
These conditions
imply that the marginal cost of measurement must be lower than both
the expected quality loss from giving and the marginal donation net
of the expected disbursal. If these conditions are not met, the entrepreneur will
increase $n$ by raising $g$. 

At the optimum, there are two possible outcomes. The first, $m_{mec}^{*}$,
satisfies what I call the ``measurement effectiveness constraint,''
or quality-margin condition.
Beyond this point, the measurement costs of employing more beneficiaries
exceed the gains in effectiveness, $\text{i.e., }\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}$,
even if more donations are available. This is given by: 

\begin{equation}
C'(m_{mec}^{*})=\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}<D'(m_{mec}^{*})-\left(\bar{a}-\mathbb{E}(a)\right).\label{eq:4}
\end{equation}
The second, $m_{mcc}^{*}$, satisfies the ``measurement compensation
constraint,'' or funding-margin condition. Here, the entrepreneur requires that part of the donations,
net of the disbursal to beneficiaries, covers the costs incurred
in performing the measurement function. This is expressed as:

\begin{equation}
C'(m_{mcc}^{*})=D'(m_{mcc}^{*})-\left(\bar{a}-\mathbb{E}(a)\right)<\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}.\label{eq:5}
\end{equation}
...
Moreover, equations (\ref{eq:4}) and (\ref{eq:5}) imply that I
can infer the entrepreneur's optimal choices for $m^{*}$, $g^{*}$,
and $n^{*}$ based on the parameters of the distribution of $a$,
$\gamma$, and the legal form ($v$). When $Var(a)$ and $\gamma$ are
sufficiently large, the measurement compensation constraint binds.
In that regime the entrepreneur serves beneficiaries only through
transactions, so
\[
m^{*}=m_{mcc}^{*}=\text{max}(m),\qquad n_{mcc}^{*}=m_{mcc}^{*},\qquad g_{mcc}^{*}=0.
\]
When instead the measurement effectiveness constraint binds first,
the entrepreneur increases $n$ through giving until
\[
D'(n_{mec}^{*})=\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}+\left(\bar{a}-\mathbb{E}(a)\right).
\]
This implies that $n_{mec}^{*}$ decreases as $Var(a)$ and $\gamma$
increase. Conversely, $m_{mec}^{*}$ increases with $Var(a)$ and $\gamma$
because $C(m)$ is continuous and convex up to $\text{max}(m)=m_{mcc}^{*}$.
When $m^{*}=m_{mcc}^{*}=n_{mcc}^{*}$,
\[
D'(n_{mcc}^{*})<\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}+\left(\bar{a}-\mathbb{E}(a)\right),
\]
which implies that $n_{mcc}^{*}>n_{mec}^{*}$ because $D(n)$ is concave.
\lemref{lem:mapping} and \propref{prop:comparative-statics} summarize these comparative statics.
\begin{lem}\label{lem:mapping}
There exists a one-to-one mapping between $m^{*}$ and $n^{*}$, $m^{*}$and
$g^{*}$, and $g^{*}$and $n^{*}$$.$
\end{lem}
...
\textbf{(B)} When $Var(a)$ and $\gamma$ are small, such that $m^{*}=m_{mec}^{*}$,
where $C'(m_{mec}^{*})=\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}<D'(m_{mec}^{*})-\left(\bar{a}-\mathbb{E}(a)\right)$,
...
\textbf{(C) }When $Var(a)$ and $\gamma$ are large, such that $m^{*}=m_{mcc}^{*}$,
where $C'(m_{mcc}^{*})=D'(m_{mcc}^{*})-\left(\bar{a}-\mathbb{E}(a)\right)<\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}$,
the total number of beneficiaries the entrepreneur chooses to assist
($n_{mcc}^{*}$) is equal to the number of beneficiaries the entrepreneur
transacts with ($m_{mcc}^{*}$), and the entrepreneur does not engage
in giving ($g_{mcc}^{*}=0$). 

\textbf{(D)} In the pure-transacting regime, $n^{*}=m^{*}=m_{mcc}^{*}$
...
Using \lemref{lem:tailoring} and equation (\ref{eq:2}), expected quality is: $\mathbb{E}\left[q\right]=-g^{*}\left(Var(a)+\frac{v^{2}}{4\gamma^{2}}\right)$.
...
as $n,m\rightarrow m_{mcc}=n_{mcc}$.\footnote{The relationship between expected quality and $Var(a)$ is defined
...
$D'(n_{mec}^{*})=\left(\bar{a}-\mathbb{E}(a)\right)+\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}$,
...
When $n^{*}=m^{*}=m_{mcc}^{*}$, the expected quality of philanthropy
is maximized. Conversely, when $m^{*}=m_{mec}^{*}$ and $Var(a)$
...
respectively. A critical factor in comparing the two forms is that
the maximum threshold for measurement costs is higher for nonprofit
entrepreneurs; specifically, $\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}>\gamma\times Var(a)-\frac{1}{4\gamma}$
(see equation (\ref{eq:4})). The reason is that the non-distribution
constraint amplifies the weight of the altruistic component in the
entrepreneur's utility function. As illustrated in Figure 2, three
main cases arise depending on the levels of $\gamma$ and $Var(a)$:
...
\textbf{Case 1. Pure Giving:} A firm will engage exclusively in giving
if the measurement effectiveness constraint in equation (\ref{eq:4})
is not satisfied for any $m$. Suppose $C'(0)>\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}$.
...
($m_{fp}^{*}=0$) because $\frac{\gamma}{v}\times Var(a)-\frac{v}{4\gamma}>\gamma Var(a)-\frac{1}{4\gamma}$,
...
by $D'(n_{np}^{*})=\bar{a}-\mathbb{E}(a)+\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}.$
For for-profits, $D'(n_{fp}^{*})=\bar{a}-\mathbb{E}(a)+\gamma Var(a)-\frac{1}{4\gamma}<D'(n_{np}^{*})$.
...
\textbf{Case 2. Mixed Giving and Transacting:} A firm will combine
giving and transacting when $Var(a)$ and $\gamma$ are moderate such
that the benefits of measurement are limited, and the entrepreneur
continues to allocate donations beyond the point at which measurement
is effective. Suppose that $C'(m^{*})=\gamma Var(a)-\frac{1}{4\gamma}$
satisfies the measurement effectiveness constraint for the for-profit
firm (see equation (\ref{eq:4})). This means that the for-profit
entrepreneur will continue to increase $n$ until $n$ satisfies the
equation $D'(n_{fp}^{*})=\bar{a}-\mathbb{E}(a)+\gamma Var(a)-\frac{1}{4\gamma}$.
Thus, the entrepreneur will mix employing the beneficiaries and giving
to them. At $m=m_{fp}^{*}$, the nonprofit entrepreneur will continue
to increase the number of beneficiaries she employs because $C(m)$
...
in transacting if the measurement compensation constraint is satisfied.} It follows that $g_{fp}^{*}>g_{np}^{*}\geq0$ because $n=m+g$. Accordingly,
$\mathbb{E}\left[q_{np}\right]>\mathbb{E}\left[q_{fp}\right]$.

\textbf{Case 3. Pure Transacting:} A firm engages exclusively in transacting
when $\gamma$ and $Var(a)$ are high, such that the measurement compensation
constraint in equation (\ref{eq:5}) is satisfied, i.e., $C'(m^{*})=D'(\text{\ensuremath{m^{*}}})-\left(\bar{a}-\mathbb{E}(a)\right)$.
As $D(n)$ is concave, the marginal income from donations decreases
with $n$, and at high levels of $\gamma Var(a)$, the measurement
compensation constraint will bind. When the informational advantages
are significant, social enterprises focus exclusively on transacting.
Importantly, the measurement compensation constraint is identical
...
The analysis in this paper offers a simple method of quality assurance
through certification, not of the quality of philanthropy
directly (which is unknown ex ante), but of the entrepreneur\textquoteright s
...
beneficiaries, provided she is compensated for measurement and disbursal
costs, such that $D'(m^{*})\geq C'(m^{*})+\left(\bar{a}-\mathbb{E}(a)\right)$.
The quality of philanthropy is then maximized ($\mathbb{E}\left[q\right]=0$)
because the entrepreneur, incentivized by revenue, sets $d_{i}^{m}=\bar{a}-a(i)\;\forall i$,
as failing to do so reduces revenue (see \lemref{lem:tailoring}). The donor sets
the donation amount according to his own altruism: $D'(m^{*})=F'(m^{*})$.
...
\multicolumn{2}{@{}l@{}}{{\large\textbf{1. Variables}}}\tabularnewline
 & \tabularnewline
$a$ & The level of abilities of each beneficiary\tabularnewline
$\bar{a}$ & The maximum level of abilities for each beneficiary\tabularnewline
$C(m)$ & The costs of measurement\tabularnewline
$d(i)$ & The amount of disbursals to each beneficiary\tabularnewline
$d^{m}(i)$ & The amount of disbursals to each beneficiary the firm transacts with\tabularnewline
$d^{g}(i)$ & The amount of disbursals to each beneficiary the firm does not transact with\tabularnewline
...
$r$ & The return from employing one worker independent of the worker's abilities\tabularnewline
...
\multicolumn{2}{@{}l@{}}{{\large\textbf{2. Subscripts}}}\tabularnewline
 & \tabularnewline
$fp$ & The allocations of the firm ($n_{fp},m_{fp}$ and $g_{fp})$ when the entrepreneur forms a for-profit\tabularnewline
$np$ & The allocations of the firm ($n_{np},m_{np}$ and $g_{np})$ when the entrepreneur forms a nonprofit\tabularnewline
$mcc$ & The allocations of the firm ($n_{mcc},m_{mcc}$ and $g_{mcc})$ when the measurement compensation constraint in equation (\ref{eq:5}) is met\tabularnewline
$mec$ & The allocations of the firm ($n_{mec},m_{mec}$ and $g_{mec})$ when the measurement compensation constraint in equation (\ref{eq:4}) is met\tabularnewline
\end{tabularx}
\end{table}
...
unmeasured transactional beneficiary, the entrepreneur weakly prefers
to reclassify that beneficiary as giving. It follows that no optimum
contains unmeasured transacting. Therefore $m_{2}^{*}=0$, and any
beneficiary served through a transaction is measured and, by \lemref{lem:tailoring},
tailored.
...
\]
In the mixed regime, the measurement-effectiveness constraint binds,
so
\[
C'(m^{*})=S(v),\qquad D'(n^{*})=\left(\bar{a}-\mathbb{E}(a)\right)+S(v).
\]
Because $C'$ is increasing and $D'$ is decreasing, $m^{*}$ and $n^{*}$
are each uniquely pinned down by the same scalar $S(v)$. Hence, in
the mixed regime, there is a one-to-one mapping between $m^{*}$ and
$n^{*}$. Since $g^{*}=n^{*}-m^{*}$, $g^{*}$ is pinned down as well.
In the pure-transacting regime, the measurement-compensation constraint
binds, so $n^{*}=m^{*}$ and $g^{*}=0$. Thus, conditional on parameters
and the relevant regime, $m^{*}$, $n^{*}$, and $g^{*}$ jointly determine
one another.
...
Once $S(v)$ becomes large enough that the measurement-compensation
constraint binds before the measurement-effectiveness constraint, the
...
Part (C) follows directly from equation (\ref{eq:5}). When the measurement-compensation
constraint binds, the entrepreneur stops exactly where the last transacted
...
In Case 3 (pure transacting), the measurement-compensation constraint
is
\[
...
help. Let $x(i)\equiv\bar{a}-a(i)\geq0$ denote beneficiary need and
replace the quadratic loss by
\[
q=-\mathbb{E}\int_{0}^{m}\left(x(i)-d^{m}(i)\right)_{+}^{2}di-\mathbb{E}\int_{0}^{g}\left(x(i)-d^{g}(i)\right)_{+}^{2}di,
\]
where $(z)_{+}\equiv\max\{z,0\}$. Measured transacting is unchanged:
if the entrepreneur observes $x(i)$, then choosing $d^{m}(i)<x(i)$
reduces both revenue and quality, while choosing $d^{m}(i)>x(i)$ only
raises costs. Hence $d^{m*}(i)=x(i)$ still holds.

Giving is different. For any beneficiary served through giving, the
entrepreneur chooses a common disbursal $d$ to solve
\[
\max_{d\geq0}\;-vd-\gamma\mathbb{E}\left[(x-d)_{+}^{2}\right].
\]
This objective is concave, with first-order condition
\[
2\gamma\mathbb{E}\left[(x-d^{g*})_{+}\right]=v
\]
for an interior solution, and boundary solution $d^{g*}=0$ if $2\gamma\mathbb{E}[x]\leq v$.
...
is now governed by the upper tail of unmet need, $\mathbb{E}\left[(x-d^{g*})_{+}^{2}\right]$,
...
$g$, or $d$. By \lemref{lem:mapping}, however, there is a one-to-one mapping from
$n^{*}$ to $m^{*}$ and $g^{*}$ within each regime. In the mixed region,
the donor can therefore infer expected quality and the entrepreneur's
true $\gamma$ from observed scale. In the pure-transacting region,
...
because funding must be chosen before $n^{*}$ is observed. Instead,
...
$v\times\kappa=\kappa>1$. When the measurement compensation constraint
binds without a tax exemption, a sufficiently large $\kappa$ may
...
The measurement-effectiveness threshold
\[
...
or through giving. By contrast, the measurement-compensation condition
in the pure-transacting region becomes
\[
C'(m^{*})=\lambda D'(m^{*})-\left(\bar{a}-\mathbb{E}(a)\right).
\]
Hence a higher $\lambda$ raises the scale of pure transacting as long
as the resulting allocation still satisfies the effectiveness bound
$C'(m^{*})<S(v)$.

Let $\bar{\lambda}$ denote the value at which the pure-transacting
solution just reaches the mixed-regime boundary:
\[
\bar{\lambda}D'(m_{mec}^{*})=S(v)+\left(\bar{a}-\mathbb{E}(a)\right),
\]
where $m_{mec}^{*}$ solves $C'(m_{mec}^{*})=S(v)$. For $\lambda<\bar{\lambda}$,
...
increases up to the point where the measurement compensation constraint
is satisfied. The graphs show the tradeoff between scale and quality.
...
the measurement-effectiveness constraint becomes binding, and further increases in $\lambda$ cause
