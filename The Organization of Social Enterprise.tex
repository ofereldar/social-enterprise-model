...
\end{prop}
This proposition also yields a simple trade-off between quality and scale.
\begin{cor}\label{cor:quality-scale}
In the mixed regime, an increase in the entrepreneur's concern for
quality ($\gamma$) increases expected quality but decreases total scale
$n^{*}$. Expected quality is maximized at the pure-transacting allocation,
where $n^{*}=m^{*}=m_{F}^{*}$, $g^{*}=0$, and $\mathbb{E}[q]=0$.
\end{cor}
The intuition is simple. An entrepreneur who places greater weight on
effectiveness shifts from giving toward measured transactions. Targeting
improves, but fewer beneficiaries are served. In the mixed regime,
\lemref{lem:tailoring} and equation (\ref{eq:2}) imply
\[
\mathbb{E}[q]=-g^{*}\left(Var(a)+\frac{v^{2}}{4\gamma^{2}}\right).
\]
An increase in $Var(a)$ has the same directional effect on the entrepreneur's
choices: $m^{*}$ rises, while $g^{*}$ and $n^{*}$ fall. Unlike an increase
in $\gamma$, however, greater heterogeneity also makes each remaining
untailored disbursal less accurate, so its net effect on quality need not
be monotonic. \exampleref{ex:oa-tradeoff} makes this trade-off concrete.
Expected quality initially falls over a small low-variance range. It then
rises as measured transactions replace giving and total scale falls,
until the pure-transacting allocation is reached. Large corporations that market fair-trade
...
In the mixed regime, \propref{prop:comparative-statics} implies that
$\partial n^{*}/\partial\gamma<0$ and $\partial g^{*}/\partial\gamma<0$.
Because
\[
\mathbb{E}[q]=-g^{*}\left(Var(a)+\frac{v^{2}}{4\gamma^{2}}\right),
\]
...
\[
\frac{\partial\mathbb{E}[q]}{\partial\gamma}
=-\frac{\partial g^{*}}{\partial\gamma}
\left(Var(a)+\frac{v^{2}}{4\gamma^{2}}\right)
+\frac{g^{*}v^{2}}{2\gamma^{3}}>0.
\]
Thus, expected quality rises while total scale falls. At the pure-transacting
allocation, $g^{*}=0$, so expected quality equals its maximum value of
zero.
...
As $\gamma$ increases, the entrepreneur chooses a larger $m$ and smaller
$g$ and $n$. Expected quality rises until the funding limit is reached,
where $g^{*}=0$ and $n^{*}=m^{*}$. Changes in $Var(a)$ produce the
same allocation pattern, but the quality effect differs at very low
levels of heterogeneity. In that range, expected quality initially falls
as $Var(a)$ rises because giving performs well when there is little need
to tailor disbursals. Beyond that range, the shift from giving to measured
transactions dominates: expected quality rises while total scale falls,
until the pure-transacting allocation is reached. The graphs therefore
illustrate the trade-off between scale and quality. When tailoring is
more important or the entrepreneur cares more about effectiveness, she
concentrates resources on assisting fewer beneficiaries rather than
spreading donations across a larger group.
\end{example}
$\,$
