...
are nonprofits, but many others operate as for-profits while hiring
disadvantaged workers, sourcing from low-income producers, or lending
to underserved borrowers. The paper starts from a practical puzzle.
In many development settings, effective assistance requires knowing
beneficiaries' heterogeneous abilities and needs, but that information
is difficult to observe before the organization works with them. Why,
then, do some social enterprises organize around transactions with
beneficiaries, and why can the for-profit form arise precisely where
tailored assistance matters most?
...
helps commit the producer not to exploit them. The problem here is
...
illustrate the same broad mechanism more loosely: repeated transactions
with low-income farmers or borrowers can reveal information that helps
tailor assistance.\footnote{For a discussion of the definition of social enterprise and of the
...
in a narrow targeting sense: the extent to which assistance is matched
...
The legal-form result is a consequence of this assistance-technology
choice. When beneficiary heterogeneity is low, a nonprofit can improve
...
The paper also relates to recent influential models of socially oriented
firms, including \citet{baron2007}, \citet{besleyGhatak2017},
\citet{deBettigniesRobinson2018}, and \citet{kaulLuo2018}, but emphasizes
a different mechanism: beneficiary-facing transactions as a source
of information that improves the targeting of philanthropy.
...
The organization also receives support tied to promised scale $n$.
I write this support as $D(n)$, where $D(n)$ is increasing, concave,
twice differentiable, and satisfies $D(0)=0$. The notation is deliberately
reduced form. What matters for the main tradeoff is that assisting
more beneficiaries attracts additional resources, but at a declining
marginal rate. This support may come from external donors, consumer
premia, investors accepting below-market returns, workers accepting
lower salaries, or the entrepreneur herself. The reduced-form schedule
also captures the realistic case in which outside supporters can observe
promised scale more easily than they can observe the quality of disbursals
ex ante \citep{charlesKim2016,coupetSchehl2022}. It can be viewed
as a tractable simplification of a fuller donor problem in which donor
utility takes the form $U^{D}=Y-D(n)+F(n)$, where $F(n)$ is increasing
and concave in the number of beneficiaries assisted. In the baseline
model, the entrepreneur chooses $n$ and takes $D(n)$ as given. \secref{subsec:donor-decision}
later relaxes this by allowing donors to care directly about expected
quality.
