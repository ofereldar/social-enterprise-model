...
and what the donor infers about $\gamma$ from scale. Donor concern for
expected quality therefore does not yield a general legal-form ranking.
If the equilibrium mapping from $\gamma$ to $n^{*}$ is one-to-one, observed
scale reveals $\gamma$, and the donor can condition funding on the implied
quality. If types pool at the same scale, expected quality instead depends
on the donor's posterior distribution over those types. Because $Q(n,v)$
and $Q_n(n,v)$ can differ by legal
form, the induced support schedules $D_v(n)$ need not be ordered. Without
additional restrictions on the funding rule and equilibrium beliefs,
the model does not determine which legal form the entrepreneur chooses
when giving remains.
