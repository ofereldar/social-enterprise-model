...
$\gamma$ and the distribution of abilities fixed. Then
$m_{np}^{*}\geq m_{fp}^{*}$, $g_{np}^{*}\leq g_{fp}^{*}$,
$n_{np}^{*}\leq n_{fp}^{*}$, and
$\mathbb{E}[q_{np}]\geq\mathbb{E}[q_{fp}]$.
If the for-profit uses giving, $g_{fp}^{*}>0$, the inequalities for
$g^{*}$, $n^{*}$, and expected quality are strict. If the for-profit
uses pure transacting, both forms choose
$n^{*}=m^{*}=m_F^{*}$ and $g^{*}=0$, and both attain maximal expected
quality.
\end{prop}

The comparison holds the entrepreneur's concern for quality fixed.
When the for-profit still uses giving, the nonprofit's lower $v$ shifts
assistance toward measured transactions and trades scale for better
targeting. Once even the for-profit uses pure transacting, transactions
align monetary and targeting incentives, so the nonprofit constraint
no longer changes the allocation. In that region, a for-profit can
match the nonprofit's targeting quality \citep{haber2016,pallotta2022}.
...
$\mathbb{E}[q_{np}]\geq\mathbb{E}[q_{fp}]$. If $g_{fp}^{*}>0$,
then $S(1)$ lies below the pure-transacting threshold. As $s$ rises
from $S(1)$, $g^{*}$ and $n^{*}$ fall strictly until that threshold
is reached and remain constant only afterward. Because $S(v)>S(1)$,
the endpoint inequalities for $g^{*}$ and $n^{*}$ are strict, as is
the expected-quality inequality.
If $g_{fp}^{*}=0$, the for-profit has reached pure transacting. Since
$S(v)>S(1)$, the nonprofit has also reached pure transacting, and both
forms choose $n^{*}=m^{*}=m_F^{*}$ and attain expected quality zero.
