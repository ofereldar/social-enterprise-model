...
\end{prop}
This proposition makes precise the trade-off between scale and targeting.
To isolate the role of heterogeneity, let $\sigma^{2}\equiv Var(a)$,
holding $\mathbb{E}(a)$ fixed. In the mixed regime,
\lemref{lem:tailoring} and equation (\ref{eq:2}) imply that expected
quality is
\[
\mathbb{E}[q]=-g^{*}\left(\sigma^{2}+\frac{v^{2}}{4\gamma^{2}}\right).
\]
An increase in $\sigma^{2}$ has two opposing effects. It increases the
expected targeting loss from each beneficiary assisted through giving.
But it also induces the entrepreneur to replace giving with measured
transactions, thereby reducing $g^{*}$. The next corollary provides
the exact condition under which this reallocation effect dominates.
\begin{cor}\label{cor:quality-scale}
Holding $\mathbb{E}(a)$ fixed, an increase in $Var(a)$ increases $m^{*}$
and decreases both $n^{*}$ and $g^{*}$ in the mixed regime. Its effect
on expected quality is
\[
\frac{\partial\mathbb{E}[q]}{\partial Var(a)}=
\frac{\gamma}{v}\left(Var(a)+\frac{v^{2}}{4\gamma^{2}}\right)
\left(\frac{1}{C''(m^{*})}-\frac{1}{D''(n^{*})}\right)-g^{*}.
\]
Consequently, expected quality increases with $Var(a)$ if and only if
\[
\frac{\gamma}{v}\left(Var(a)+\frac{v^{2}}{4\gamma^{2}}\right)
\left(\frac{1}{C''(m^{*})}-\frac{1}{D''(n^{*})}\right)>g^{*}.
\]
Expected quality is maximized at the pure-transacting allocation, where
$n^{*}=m^{*}=m_{F}^{*}$, $g^{*}=0$, and $\mathbb{E}[q]=0$.
\end{cor}
The condition in \corref{cor:quality-scale} has a direct interpretation.
Greater heterogeneity improves expected quality precisely when the resulting
reduction in untailored giving more than offsets the greater targeting loss
from each remaining giving beneficiary. If the mixed regime reaches a
pure-transacting boundary, the condition holds for values of $Var(a)$
sufficiently close to that boundary because $g^{*}$ approaches zero
while the reallocation effect remains positive. In that region, the model
generates a trade-off between expected quality and scale (see \exampleref{ex:oa-tradeoff}
in \secref{oa:examples} of the Online Appendix). Entrepreneurs
...
\[
C'(m^{*})=S(v),\qquad
D'(n^{*})=\bar{a}-\mathbb{E}(a)+S(v),
\]
where $\partial S(v)/\partial\sigma^{2}=\gamma/v$. Implicit differentiation
therefore gives
\[
\frac{\partial m^{*}}{\partial\sigma^{2}}
=\frac{\gamma/v}{C''(m^{*})}>0
\qquad\text{and}\qquad
\frac{\partial n^{*}}{\partial\sigma^{2}}
=\frac{\gamma/v}{D''(n^{*})}<0.
\]
Because $g^{*}=n^{*}-m^{*}$,
\[
\frac{\partial g^{*}}{\partial\sigma^{2}}
=\frac{\gamma}{v}\left(\frac{1}{D''(n^{*})}-\frac{1}{C''(m^{*})}\right)<0.
\]
Differentiating expected quality and substituting this expression yields
\[
\frac{\partial\mathbb{E}[q]}{\partial\sigma^{2}}
=\frac{\gamma}{v}\left(\sigma^{2}+\frac{v^{2}}{4\gamma^{2}}\right)
\left(\frac{1}{C''(m^{*})}-\frac{1}{D''(n^{*})}\right)-g^{*},
\]
which establishes the condition in the corollary. At the pure-transacting
allocation, $g^{*}=0$, so expected quality equals its maximum value of
zero. If the mixed regime approaches a pure-transacting boundary, continuity
and the curvature conditions stated above imply that the positive reallocation
term remains strictly positive as $g^{*}$ approaches zero. Expected quality
therefore increases with $Var(a)$ sufficiently close to that boundary.
