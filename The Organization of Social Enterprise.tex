...
in scale. Subscripts on $F$ and $Q$ denote partial derivatives. The scale-only
benchmark sets $F_Q=0$ and writes the resulting benefit as
$F(n)=F(n,0)$. At a differentiable interior solution, the donor's marginal
condition is
\[
...
\]
At this allocation, $Q(\widehat n,v)=Q_n(\widehat n,v)=0$, so
$D_v'(\widehat n)=F_n(\widehat n,0)$ and scale is unchanged from the
scale-only benchmark.
Absent tax benefits,
$U_{fp}^{E}>U_{np}^{E}$ for every $\gamma\in\Gamma$.

\textbf{(B)} For a type $\gamma\in\Gamma$, at any differentiable interior
equilibrium in which $g^{*}>0$ under legal-form parameter $v$, scale satisfies
\[
...
The sign of $U_{fp}^{E}-U_{np}^{E}$ is therefore not determined when
giving remains under at least one form.
\end{prop}

The left side of the condition in part (A) is $S(1)$ evaluated at
$\gamma=\underline{\gamma}$, the targeting advantage for the lowest-$\gamma$
for-profit type. It increases with $Var(a)$. Since $S(v)$ increases in
...
$\gamma$. Because $C'$ is increasing, the same condition holds at every
$n\leq\widehat n$. Together with $D_v(0)=0$, the common marginal funding
condition implies equal support under the two legal forms at $\widehat n$.
...
For part (A), the left side of the stated inequality is $S(1)$ for
$\gamma=\underline{\gamma}$. Because $S(v)$ increases with $\gamma$
and decreases with $v$, the inequality implies
...
across legal forms. The legal-form ranking is therefore not determined
