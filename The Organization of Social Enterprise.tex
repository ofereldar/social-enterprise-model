...
\end{cor}
The corollary does not assume that conventional firms are unable to
learn about beneficiaries. It identifies when beneficiary-facing transactions
matter in the model. If ability can be observed without transacting,
...
monetary payoff is positive at each form's optimum.

\textbf{(A)} If $S(1)\geq C'(m_F^{*})$, both forms choose the same
pure-transacting allocation and attain maximal expected quality, but
the entrepreneur strictly prefers the for-profit.

\textbf{(B)} If $S(1)<C'(m_F^{*})$, the for-profit uses giving. The
nonprofit would produce higher expected quality at lower scale, but
the entrepreneur strictly prefers the for-profit.
\end{prop}
...
does not assume that the for-profit entrepreneur has a lower $\gamma$.
