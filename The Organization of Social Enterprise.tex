...
of jobs does not constrain transacting.\footnote{For the values of $m$ and $n$ considered below, I assume that $C$ and $D$ are twice continuously differentiable, $C''(m)>0$, $D''(n)<0$, $\lim_{n\rightarrow\infty}D'(n)=0$, and equations (\ref{eq:4}) and (\ref{eq:5}) have finite solutions when they apply. If the optimal disbursal to each beneficiary served through giving is zero, $d^{g*}(i)=0$, the calculations below change. If $m^{*}=0$, the pure-giving case applies, as discussed in \secref{subsec:legal-form-effects}. If $m^{*}=w$, the fixed number of jobs limits transacting.}
...
One of two limits stops expansion through transactions.
