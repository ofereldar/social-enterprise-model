...
the formal proofs for the core lemmas and the main comparative statics.
...
and giving. For the comparative statics below, I assume that the giving
disbursal is positive, $\bar{a}-\mathbb{E}(a)>v/(2\gamma)$, and focus
on a finite, unique optimum with nonbinding production
capacity.\footnote{Over the relevant range, I assume that $C$ and $D$ are twice continuously differentiable, $C''(m)>0$, $D''(n)<0$, $\lim_{n\rightarrow\infty}D'(n)=0$, and the relevant first-order conditions have solutions. If the giving-disbursal condition fails, $d^{g*}(i)=0$ and the marginal expressions governing scale change. If $m^{*}=0$, the pure-giving case applies, as discussed in \secref{subsec:legal-form-effects}. If capacity binds, transacting is capped at $m^{*}=w$.}
...
Holding $\mathbb{E}(a)$ fixed, suppose that the giving disbursal is
positive, the optimum is finite and unique, $C''(m)>0$, $D''(n)<0$,
and production capacity does not bind. Then:
