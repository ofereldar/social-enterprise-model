...
While many donors may be passive and rely primarily on observable scale,
many others care directly about the expected quality of the programs
they support. \oaref{oa:donors-quality} develops this extension formally.
Expected quality depends in part on $\gamma$, the weight the entrepreneur
places on effective assistance. The entrepreneur knows her own $\gamma$,
but the donor does not observe it. The donor observes legal form and
the entrepreneur's planned philanthropic scale $n$ ex ante, but not
...
Observable scale nevertheless contains information whenever some giving
remains. In the mixed regime, \propref{prop:comparative-statics} implies
that a higher $\gamma$ leads to a lower $n^{*}$, while
\lemref{lem:mapping} establishes a one-to-one mapping among $n^{*}$,
$m^{*}$, and $g^{*}$. Thus, when entrepreneur types choose distinct
planned scales, observed scale identifies $\gamma$, the allocation,
and expected quality, conditional on legal form and the known funding
schedule. This is a separating equilibrium. Under pure giving,
$m^{*}=0$ and $g^{*}=n^{*}$, and the scale condition is likewise strictly
monotone in $\gamma$, so the same inference follows. In the pure-transacting
region, different types may instead choose the same scale, so $n^{*}$
need not reveal the exact $\gamma$. Exact identification is unnecessary
there because every type sets $g^{*}=0$ and achieves maximal quality.
...
As \corref{cor:quality-scale} shows, the highest-quality positive-scale
outcome in the model is pure transacting. When the entrepreneur relies
entirely on transactions with beneficiaries, expected quality is maximized,
but further expansion through giving would reduce quality. Limited scale
therefore reflects a commitment to quality. The entrepreneur forgoes
the funding associated with assisting additional beneficiaries rather
than expand through untailored giving. This creates a scaling challenge,
which is consistent with recurring concerns about the difficulty social
enterprises face in scaling their impact.

The preceding analysis assumes that planned scale $n$ is observable.
In a separating equilibrium in which some giving remains, scale allows
donors to infer the entrepreneur's $\gamma$. A higher-$\gamma$ entrepreneur
places more weight on targeting quality, relies less on giving, and
chooses lower total scale. Observably limited scale therefore signals
a stronger concern for quality. In the pure-transacting region, scale
may not reveal the entrepreneur's exact $\gamma$, but that information
is unnecessary because every type provides maximal quality. These results
are developed formally in \oaref{oa:donors-quality}.
Certification becomes valuable when planned scale is not verifiable
and donors cannot otherwise infer quality, or more generally whenever
scale cannot perform this informational role.
...
about expected quality, provided all entrepreneur types in the donor's
prior support lie strictly within the pure-transacting region. When
