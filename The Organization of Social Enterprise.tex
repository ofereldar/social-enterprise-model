...
and giving. For the comparative statics below, I assume that each beneficiary
served through giving receives a positive disbursal,
$\bar{a}-\mathbb{E}(a)>v/(2\gamma)$, and focus
on a finite, unique optimum with $m^{*}<w$, so the firm's fixed number
of jobs does not constrain transacting.\footnote{For the values of $m$ and $n$ considered below, I assume that $C$ and $D$ are twice continuously differentiable, $C''(m)>0$, $D''(n)<0$, and $\lim_{n\rightarrow\infty}D'(n)=0$. If the optimal disbursal to each beneficiary served through giving is zero, $d^{g*}(i)=0$, the calculations below change. If $m^{*}=0$, the pure-giving case applies, as discussed in \secref{subsec:legal-form-effects}. If $m^{*}=w$, the fixed number of jobs limits transacting.}
...
Holding $\mathbb{E}(a)$ fixed, suppose that each beneficiary served through
giving receives a positive disbursal, the optimum is finite and unique,
$C''(m)>0$, $D''(n)<0$,
and $m^{*}<w$. Then:
...
fewer beneficiaries. Large corporations that market fair-trade
...
are redundant in this case. The entrepreneur can tailor disbursals under
giving to each beneficiary, choosing
\[
