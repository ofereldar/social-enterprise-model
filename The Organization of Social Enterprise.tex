...
The entrepreneur also chooses legal form and, after the relevant information
is learned, disbursals $d(i)$. The firm employs a fixed number of
workers $w$; $w$ is not a choice variable.
...
is what the entrepreneur learns. If the entrepreneur helps $g$ beneficiaries through giving,
...
is wasteful. Let $r>1$ denote the return per unit of worker ability.
If the firm employs only standard workers,
...
pressure, or both. In contrast, \citet{glaeserShleifer2001}
...
The model's central alignment is easiest to see by separating quality
losses for transacted and giving beneficiaries:
\[
q=-\int_{0}^{m}\left(d^{m}(i)-(\bar{a}-a(i))\right)^{2}di-\int_{0}^{g}\left(d^{g}(i)-(\bar{a}-a(i))\right)^{2}di.
\]
Maximum philanthropic quality is achieved when $d(i)=\bar{a}-a(i)$
for every assisted beneficiary, so $q=0$. For beneficiaries employed
inside the firm, the same tailored disbursal also raises productive
output up to the standard-worker level. Thus, when the entrepreneur
transacts with beneficiaries and learns their abilities, the revenue
incentive and the quality incentive point in the same direction.

Putting these pieces together, the entrepreneur chooses $m$, $g$,
and disbursals to solve:
...
The bracketed term is the net monetary payoff, consisting of productive
revenue plus donations, less disbursals and measurement costs. The last term is
the quality payoff. Productive revenue depends on $m$ and on the
transactional disbursals chosen by the entrepreneur:
...
(\ref{eq:2}).} Thus, the more the entrepreneur values effectiveness, the closer
...
to beneficiaries\textquoteright{} needs at $t=1$. Thus,
...
channel. Each increase in scale $n$ can be implemented either by
raising $m$ or by raising $g$. Raising $m$ expands scale through
...
$(n',m',g')$, expected utility in equation (\ref{eq:-1}) can be
rewritten as
...
\]
The entrepreneur will expand through transactions rather than giving
only if
\[
...
At the optimum, one of two constraints stops expansion through transactions.
The first, $m_{mec}^{*}$,
...
even if more donations are available:
...
in performing the measurement function:
...
transactions, so
\[
m^{*}=m_{mcc}^{*},\qquad n_{mcc}^{*}=m_{mcc}^{*},\qquad g_{mcc}^{*}=0.
\]
...
because $C(m)$ is continuous and convex over the relevant range.
...
There exists a one-to-one mapping between $m^{*}$ and $n^{*}$, between
$m^{*}$ and $g^{*}$, and between $g^{*}$ and $n^{*}$.
\end{lem}
...
$\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}>\gamma Var(a)-\frac{1}{4\gamma}$
...
is never worth its cost. Formally, the measurement effectiveness constraint
in equation (\ref{eq:4}) is not satisfied for any $m$. Suppose
$C'(0)>\frac{\gamma}{v}Var(a)-\frac{v}{4\gamma}$.
...
all additional scale. Suppose that $C'(m^{*})=\gamma Var(a)-\frac{1}{4\gamma}$
...
enough that the entrepreneur avoids giving altogether. The measurement
compensation constraint in equation (\ref{eq:5}) is then satisfied,
...
$mcc$ & The firm allocations ($n_{mcc}$, $m_{mcc}$, and $g_{mcc}$) when the measurement compensation constraint in equation (\ref{eq:5}) is met\tabularnewline
$mec$ & The firm allocations ($n_{mec}$, $m_{mec}$, and $g_{mec}$) when the measurement effectiveness constraint in equation (\ref{eq:4}) is met\tabularnewline
\end{tabularx}
\end{table}
