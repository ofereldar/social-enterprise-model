...
of jobs does not constrain transacting.\footnote{Over the relevant range, I assume that $C$ and $D$ are twice continuously differentiable, $C''(m)>0$, $D''(n)<0$, $\lim_{n\rightarrow\infty}D'(n)=0$, and the relevant first-order conditions have solutions. If the optimal disbursal to each beneficiary served through giving is zero, $d^{g*}(i)=0$, the calculations below change. If $m^{*}=0$, the pure-giving case applies, as discussed in \secref{subsec:legal-form-effects}. If $m^{*}=w$, the fixed number of jobs limits transacting.}
...
$(n',m',g')$ under these assumptions, substituting the optimal disbursal
rules into equation (\ref{eq:-1}) gives
...
rises while $g^{*}$ and total scale $n^{*}$ fall. If $Var(a)$ becomes
high enough to induce pure transacting, optimal scale reaches its minimum,
$n^{*}=m^{*}=m_{F}^{*}$, while expected quality reaches its maximum,
$\mathbb{E}[q]=0$.
\end{cor}
...
\]
Expected quality need not rise at every point in the mixed regime because
greater heterogeneity also makes each remaining untailored disbursal less
accurate. \exampleref{ex:oa-tradeoff} shows the full path. Quality falls
slightly over a narrow low-variance range, but then rises as measured
transactions replace giving and total scale falls, reaching its maximum
at the pure-transacting allocation. This is the central trade-off: higher
quality is achieved by serving fewer beneficiaries. Large corporations that market fair-trade
...
In the mixed regime, $S(v)$ increases with both $Var(a)$ and $\gamma$.
Because $C'(m^{*})=S(v)$ and $C'$ is increasing, $m^{*}$ increases.
Because $D'(n^{*})=\bar{a}-\mathbb{E}(a)+S(v)$ and $D'$ is decreasing,
$n^{*}$ decreases. It follows that $g^{*}=n^{*}-m^{*}$ also decreases.
These observations establish parts (A) and (B).

Part (C) follows from equation (\ref{eq:5}). When the funding
...
$n^{*}=m^{*}=m_{F}^{*}$, and every assisted beneficiary is served through
a transaction.

\subsection{Proof of \corref{cor:quality-scale}}

Let $\sigma^{2}=Var(a)$ and hold $\mathbb{E}(a)$ fixed. In the mixed
regime, \propref{prop:comparative-statics} implies that $m^{*}$ rises,
while $g^{*}$ and $n^{*}$ fall, as $\sigma^{2}$ increases. Expected
quality is
\[
\mathbb{E}[q]=-g^{*}\left(\sigma^{2}+\frac{v^{2}}{4\gamma^{2}}\right).
\]
The direct effect of greater variance makes untailored disbursals less
accurate, while the reduction in $g^{*}$ improves quality. The net effect
need not have the same sign throughout the mixed regime. If $\sigma^{2}$
becomes high enough that the funding limit binds first, however,
\propref{prop:comparative-statics} implies $g^{*}=0$ and
$n^{*}=m^{*}=m_{F}^{*}$. Expected quality then equals its maximum value
of zero. Because $n^{*}$ decreases throughout the mixed regime and reaches
$m_{F}^{*}$ at the pure-transacting boundary, this allocation also has
the minimum optimal scale along this comparative-static path.
