...
The nonprofit form therefore gives the entrepreneur a
greater marginal incentive to replace giving with measured transactions.
Under the conditions of \propref{prop:comparative-statics}, the choices
...
\]
The nonprofit both relies weakly less on giving and, when it gives,
chooses disbursals closer to average beneficiary need. It therefore
produces weakly higher expected targeting quality.
...
I now allow the entrepreneur to choose legal form. The central case
is the high-heterogeneity region identified in
\propref{prop:legal-form-effects}(A). When
\[
S(1)\geq C'(m_F^{*}),
\]
the for-profit and nonprofit choose the same pure-transacting allocation:
\[
n_{fp}^{*}=m_{fp}^{*}=n_{np}^{*}=m_{np}^{*}=m_F^{*},
\qquad g_{fp}^{*}=g_{np}^{*}=0.
\]
Under either form, measurement reveals each beneficiary's ability and
the entrepreneur tailors every disbursal to need. Transacting therefore
aligns monetary and targeting incentives, and both forms attain maximal
expected quality.

Legal form nevertheless affects how much of the same monetary payoff
the entrepreneur may appropriate. Let $\mathbb{E}[P_T^{*}]$ denote
the expected net monetary payoff from the common pure-transacting
allocation. Because targeting quality equals zero,
\[
U_{fp}^{E}=\mathbb{E}[P_T^{*}],
\qquad
U_{np}^{E}=v\mathbb{E}[P_T^{*}].
\]
Under the maintained nonnegativity assumption, the entrepreneur therefore
strictly chooses the for-profit whenever $\mathbb{E}[P_T^{*}]>0$ and
is indifferent only in the boundary case $\mathbb{E}[P_T^{*}]=0$.
Thus, whenever the project generates a positive monetary payoff, the
for-profit both matches the nonprofit's quality and is preferred where
beneficiary heterogeneity makes measurement and tailored assistance
most important.

For the remaining comparison, let
\[
M(m,g)\equiv rw\bar a+D(m+g)-C(m)
-(m+g)\left(\bar a-\mathbb{E}(a)\right)
\]
denote the monetary component in equation (\ref{eq:3}), and assume
that it is nonnegative at each form's optimum.
...
the nonnegative monetary component. It can then optimize further. The
formal revealed-preference argument is provided
in the proof of \propref{prop:legal-form-choice} in Section B of the
Appendix.

\begin{prop}\label{prop:legal-form-choice}
Suppose the conditions of \propref{prop:comparative-statics} hold,
nonprofits receive no tax benefits, scale is observable ex ante, and
the monetary component $M$ is nonnegative at each form's optimum.

\textbf{(A)} If $S(1)\geq C'(m_F^{*})$, both forms choose the same
pure-transacting allocation and attain maximal expected quality. The
entrepreneur strictly prefers the for-profit whenever
$\mathbb{E}[P_T^{*}]>0$ and is indifferent if
$\mathbb{E}[P_T^{*}]=0$.
...
\secref{oa:examples} of the Online Appendix illustrates both regions.
...
={}&(1-v)M(m_v^{*},g_v^{*})\\
&+\frac{(1-v^{2})g_v^{*}}{4\gamma}\geq0,
\end{aligned}
\]
where the inequality follows from $v\in(0,1)$ and the maintained
nonnegativity assumption. Because the for-profit optimizes $W_1$,
\begin{equation}
U_{fp}^{E}=W_1(m_1^{*},g_1^{*})
\geq W_1(m_v^{*},g_v^{*})
\geq W_v(m_v^{*},g_v^{*})=U_{np}^{E}.
\label{eq:6}
\end{equation}
...
=(1-v)\mathbb{E}[P_T^{*}],
\]
which is positive if $\mathbb{E}[P_T^{*}]>0$ and zero otherwise.

For part (B), $S(1)<C'(m_F^{*})$ implies that the for-profit uses
giving. By \propref{prop:legal-form-effects}(B), its allocation differs
from the nonprofit allocation. Under the maintained uniqueness
condition, the first inequality in equation (\ref{eq:6}) is therefore
strict. It follows that $U_{fp}^{E}>U_{np}^{E}$.
