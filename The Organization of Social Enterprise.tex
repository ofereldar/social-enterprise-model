...
how much assistance each beneficiary needs. She can use the giving
channel, which can reach many people but must rely on imperfect information.
Or she can use the transaction channel, which reveals beneficiary-specific
information but requires costly measurement. The central tradeoff is
...
Of these $n$ beneficiaries, $m$ are served through the transaction
channel, meaning that they transact with the firm, and $g$ are served
through the giving channel, so $n=m+g$. Because beneficiaries form
a continuum, $n$, $m$, and $g$ are masses; I refer to them as numbers
throughout.
...
so $0\leq m\leq w$.\footnote{The same structure maps to other beneficiary-facing firms. In a fair-trade application, $w$ is the firm's fixed number of supplier relationships, $m$ consists of low-income farmer-suppliers, and the remaining $w-m$ are standard suppliers; revenue depends on the farmers' productivity or input quality after assistance. In microfinance, $w$ is lending capacity and revenue depends on borrowers' ability to repay. For a low-cost service provider, $w$ is the fixed number of customers the firm serves, $m$ consists of low-income customer-beneficiaries, and the remaining $w-m$ are standard customers; revenue depends on customers' ability to pay. In each application, the transaction channel both produces revenue and reveals information relevant to tailoring assistance.}
Beneficiaries served through the giving channel are assisted outside
the firm and do not occupy these jobs.
...
and its division between the transaction channel ($m$) and the giving
channel ($g=n-m$).
\item At $t=1$, the entrepreneur may incur a cost to measure the abilities
of beneficiaries served through the transaction channel. She then
chooses disbursals, and payoffs are realized.
\end{itemize}

For targeting purposes, the key difference between the giving and transaction
channels is what the entrepreneur learns. A disbursal $d(i)\geq0$
is assistance measured in units of ability. Each unit costs one and
raises ability one-for-one up to $\bar{a}$. Thus, beneficiary $i$
needs the tailored disbursal $\bar{a}-a(i)$. A smaller disbursal leaves
the beneficiary below $\bar{a}$, while a larger disbursal is wasteful.

If the entrepreneur helps $g\geq0$ beneficiaries through the giving
channel, she does not observe their individual abilities and chooses
disbursals $d^{g}(i)$ based on the distribution of needs.\footnote{The model could in principle allow partial learning through the giving channel,
...
serves $m$ beneficiaries through the transaction channel, she may incur
measurement cost $C(m)$ to observe their abilities before choosing
$d^{m}(i)$.
\lemref{lem:measure-if-transact} in \secref{subsec:transacting-giving}
shows that an entrepreneur who uses the transaction channel always
measures each beneficiary's ability, $a(i)$, and tailors the beneficiary's
disbursal to that ability.
...
The model's central alignment is now immediate. For a beneficiary served
through the giving channel, the entrepreneur must choose $d^{g}(i)$
without knowing $a(i)$. For a beneficiary served through the transaction
channel, measurement reveals $a(i)$, and the tailored disbursal $d^{m}(i)=\bar{a}-a(i)$
both raises productive revenue and maximizes targeting quality. Profit
and mission therefore point in the same direction when the firm uses
