...
firm employs a fixed number of workers $w$; $w$ is not a choice variable.
...
The two ways of helping beneficiaries differ only in what the entrepreneur
...
and satisfies $D(0)=0$. The notation is deliberately reduced form.
What matters for the main tradeoff is that assisting more beneficiaries
attracts additional donations, but at a declining marginal rate. Donations
may come from external donors, consumer premia, investors accepting
below-market returns, workers accepting lower salaries, or the entrepreneur
herself. The reduced-form schedule also captures the realistic case
...
It can be viewed as a tractable simplification of a fuller donor problem
in which donor utility takes the form $U^{D}=Y-D(n)+F(n)$, where
$F(n)$ is increasing and concave in the number of beneficiaries assisted.
In the baseline model, the entrepreneur chooses $n$ and takes $D(n)$
as given. \secref{subsec:donor-decision} later relaxes this by allowing donors to care directly
...
constraint.'' Here, the entrepreneur requires that part of the donations,
