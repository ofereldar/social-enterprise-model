...
to their abilities.
\end{lem}
The intuition is straightforward. Without measurement, the entrepreneur
...
For the interior comparative statics below, I assume that $C$ is strictly
convex, $D$ is strictly concave with $\lim_{n\rightarrow\infty}D'(n)=0$,
and the relevant displayed first-order conditions have solutions. I also assume
that $w$ is sufficiently large that the capacity constraint does not
bind. These conditions make the relevant candidate solutions finite
and unique. If capacity binds, transacting is capped at $m^{*}=w$.
If measurement is not worthwhile at $m=0$, the pure-giving corner
applies, as discussed in \secref{subsec:legal-form-effects}.
...
of the expected disbursal. The first inequality makes expansion through
transactions preferable to expansion through giving. The second ensures
that expansion through transactions has a positive marginal return.
...
At an interior optimum with $m^{*}>0$ and nonbinding capacity, one of two constraints stops expansion through transactions.
The first, $m_{mec}^{*}$,
