...
the set of possible values of $\gamma$. The entrepreneur knows her own
$\gamma$, but the donor does not observe it. The donor observes legal form and
...
Observable scale nevertheless contains information about the entrepreneur's
allocation and expected quality. In the mixed regime,
...
both scale and expected quality. Let $0<\widehat n<w$ denote the unique
solution to
\[
...
\]
Donor concern for expected quality does not alter the marginal funding
condition or equilibrium scale. Absent tax benefits,
$U_{fp}^{E}>U_{np}^{E}$ for every $\gamma\in\Gamma$.

\textbf{(B)} For any $\gamma\in\Gamma$ for which $g_j^{*}>0$ under at
least one legal form $j\in\{fp,np\}$, the maintained assumptions do
not sign $U_{fp}^{E}-U_{np}^{E}$.
\end{prop}

The condition in part (A) compares the targeting advantage of transacting
for the lowest-$\gamma$ for-profit type with the marginal cost of measurement
at $\widehat n$. Because the left side increases with $Var(a)$, the condition
holds when beneficiary heterogeneity is sufficiently high. Every higher-$\gamma$
type then chooses pure transacting, as does the nonprofit. Donors therefore
anticipate maximal quality even though common scale does not reveal the
entrepreneur's exact $\gamma$.
...
exact $\gamma$. This restraint creates the scaling challenge associated
...
Part (B) applies when some giving remains. Donor concern for expected quality
does not yield a general legal-form ranking. If different entrepreneur
types choose different planned scales, observed scale reveals $\gamma$,
and the donor can condition funding on the implied quality. If different
types choose the same scale, funding instead depends on the donor's
posterior beliefs. In either case, quality-sensitive
...
and increases as $v$ falls below one, the inequality implies
...
For part (B), let $Q(n,v)$ denote the donor's expected quality conditional
on observed scale and legal form, and let $\pi>0$ be the weight placed
on expected quality. At a differentiable interior equilibrium in which
giving remains, the donor's and entrepreneur's marginal conditions derived
in \oaref{oa:donors-quality} combine to give
\[
F'(n^{*})+\pi Q_n(n^{*},v)
=\bar a-\mathbb{E}(a)+S(v).
\]
The maintained assumptions do not restrict $Q_n(n^{*},v)$ across legal
forms because it depends on equilibrium inference and on how scale changes
giving. They therefore do not sign the entrepreneur's utility difference
across legal forms.
...
not generally determined when giving remains, establishing part (B).
