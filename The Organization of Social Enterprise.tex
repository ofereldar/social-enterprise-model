...
through giving, so $n=m+g$. Because beneficiaries form a continuum,
$n$, $m$, and $g$ are masses; I refer to them as numbers throughout.

The entrepreneur also chooses legal form. Let $v$ be the share of
the firm's monetary payoff that she values. A for-profit has $v=1$.
A nonprofit has $v\in(0,1)$, reflecting the nondistribution constraint as
a reduced-form limit on the monetary payoff the entrepreneur can appropriate.
The analysis first solves the allocation problem for a fixed $v$ and
then compares legal forms.
...
so $0\leq m\leq w$.
\footnote{The same structure maps to other beneficiary-facing firms. In a fair-trade application, $w$ is the firm's fixed number of supplier relationships, $m$ consists of low-income farmer-suppliers, and the remaining $w-m$ are standard suppliers; revenue depends on the farmers' productivity or input quality after assistance. In microfinance, $w$ is lending capacity and revenue depends on borrowers' ability to repay. For a low-cost service provider, $w$ is service capacity and revenue depends on customers' ability to pay. In each application, transacting both produces revenue and reveals information relevant to tailoring assistance.}
...
\item At $t=1$, the entrepreneur may measure the abilities of transactional
beneficiaries, chooses disbursals, and payoffs are realized.
\end{itemize}
...
serves $m$ beneficiaries through transactions, she may incur measurement
cost $C(m)$ to observe their abilities before choosing $d^{m}(i)$.
...
to a linear cost function.} Although transacting without measurement
is feasible, \lemref{lem:measure-if-transact} shows that treating such
beneficiaries as giving never lowers utility. Thus, $m$ denotes measured
transactional beneficiaries, and the reduced problem below includes
$C(m)$.

Let $r>1$ be the revenue produced by one unit of worker ability. Thus,
a unit of well-targeted assistance costs one but can raise revenue by
$r$. If the firm employs only standard workers, revenue is $rw\bar{a}$.
If it instead employs $m$ disadvantaged beneficiaries, realized revenue
is
...
Funding depends on the firm's planned scale. Let $D(n)$ be an increasing,
concave, and twice-differentiable donation schedule, with $D(0)=0$.
The concavity of $D(n)$ captures donors' limited willingness to fund
ever-larger scale. Funding may come from external donors, consumer premia,
concessionary capital or labor, or the entrepreneur herself. The schedule
also captures the realistic case in which donors can observe scale more
easily than they can assess the quality of disbursals ex ante
\citep{charlesKim2016,coupetSchehl2022}. It is a tractable simplification
of a fuller donor decision in which $U^{D}=Y-D(n)+F(n)$, where $Y$
is donor wealth and $F(n)$ is an increasing, concave benefit from the
number of beneficiaries assisted. The entrepreneur takes the schedule
$D(\cdot)$ as given. \secref{subsec:donor-decision} later allows donors
to care directly about expected quality.

The firm's realized net monetary payoff is
...
to reach $\bar{a}$. The quadratic loss treats over-provision and under-provision
symmetrically and should be read as a reduced-form measure of matching
accuracy.\footnote{\secref{oa:no-penalty} of the Online Appendix discusses a quality measure with no penalty for waste. The conclusion discusses the case in which quality is defined by quantity.}
...
The model's central alignment is now immediate. For a giving beneficiary,
the entrepreneur must choose $d^{g}(i)$ without knowing $a(i)$. For
a transactional beneficiary, measurement reveals $a(i)$, and the tailored
disbursal $d^{m}(i)=\bar{a}-a(i)$ both raises productive revenue and
maximizes targeting quality. Profit and mission therefore point in the
same direction when the firm transacts with beneficiaries.
