...
The preceding analysis assumes that planned scale is observable and that
every possible entrepreneur places some positive weight on effective
assistance. If planned scale is not verifiable, or if donors cannot rule out
a purely commercial entrepreneur,\footnote{When $\gamma=0$, the entrepreneur
sets $d^{g*}=0$ because giving produces no revenue and disbursals are costly.
She may nevertheless choose $g^{*}>0$ if scale-based funding $D(n)$ is sufficiently
valuable. Positive planned scale therefore need not imply positive assistance.} scale alone cannot assure them of expected
quality. Certification offers a different route by making beneficiary
...
incentive therefore ensures tailored certified transactions even when
$\gamma=0$. \propref{prop:certification} requires $\gamma>0$ only to rule
out additional untailored giving and guarantee maximal overall quality.

Certification of giving alone provides little assurance. For any common
...
quality. Under a fixed payment conditioned only on the number nominally
served, an entrepreneur with $\gamma=0$ sets $d=0$ and retains the payment.
Even a required positive disbursal cannot establish targeting quality because
the certifier does not observe individual need. Doing so would require
beneficiary-by-beneficiary impact assessment. Transaction certification avoids
...
guarantee a positive disbursal, much less maximal targeting quality.
\end{prop}
...
Under an analogous fixed payment conditioned only on $g$, an entrepreneur
with $\gamma=0$ sets $d=0$ because giving produces no revenue and disbursals
are costly. The certificate therefore does not ensure a positive disbursal.
Even if the contract requires $d>0$, equation
(\ref{eq:giving-certification-loss}) implies an expected targeting loss
of at least $Var(a)>0$. Guaranteeing targeting quality would require the
